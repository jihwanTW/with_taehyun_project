...
query(QueryType, [Id, Pwd, Name, Email, Nickname, Salt]) when QueryType =:= user_register ->
  % generate random idx
  Sql = "INSERT INTO user (id,pwd,name,email,nickname,salt,regist_datetime) values (?, ?, ?, ?, ?,? ,now())",
  Result = utils:query_execute(db, user_register, Sql, [Id, Pwd, Name, Email, Nickname, Salt]),
...
query(QueryType, []) when QueryType =:= board_list ->
  Sql = "SELECT idx, title, datetime, user_idx, user_nick, user_id, image_address, view_count, board_name FROM board WHERE remove = 'false'",
...
query(QueryType, [Post_idx]) when QueryType =:= board_view ->
  ...
  case Result#result_packet.rows of
    [] ->
      {error, [{<<"message">>, <<"post is not exist">>}]};
    _ ->
      %% 업데이트 보드 view_count
      ...
      {ok, [{<<"post">>, utils:query_result_to_json_list(Result)}]}
  end
;
query(QueryType, [Board, Title, Contents, User_idx]) when QueryType =:= board_write ->
  ...
  case Board1 of
    ...
    _ ->
      ...
      InsertId = Result#ok_packet.insert_id,
...
query(QueryType, [Post_idx, User_idx]) when QueryType =:= board_remove ->
  ...
  case Result_num_row of
    0 ->
      {error, [{<<"message">>, <<"error_post_not_exist">>}]};
    _ ->
...
query(QueryType, [User_idx, User_nick, Profile_image_address]) when QueryType =:= fixed_data ->
  ...
  case Result_num_row of
    0 ->
      {error, [{<<"message">>, <<"error_user_not_exist or not chagned nickname any user">>}]};
    _ ->
